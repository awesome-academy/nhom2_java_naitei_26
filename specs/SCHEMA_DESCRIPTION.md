...
Schema gồm **11 bảng** được chia thành **5 nhóm chức năng** phục vụ hệ thống đặt tour du lịch trực tuyến:

| # | Nhóm | Bảng | Mục đích |
|---|------|------|----------|
| 1 | User / Account | `users`, `social_account` | Quản lý người dùng & đăng nhập xã hội |
| 2 | Category | `category` | Phân loại tour |
| 3 | Tour System | `tour`, `tour_image`, `tour_departure` | Quản lý tour, lịch khởi hành |
...
- Bảng này được tham chiếu bởi: `social_account`, `booking`, `review_like`, `comments`.
...
- Bảng liên kết: `tour_image` (ảnh), `tour_departure` (lịch khởi hành).
...
> **Vai trò nghiệp vụ**: Bình luận trên review — hỗ trợ **cấu trúc cây (threaded comments)** qua `parent_comment_id` tự tham chiếu. User có thể comment trên review hoặc reply vào comment khác (FR-009).
...
Schema định nghĩa **11 indexes** trên tất cả foreign key để tối ưu hiệu năng truy vấn JOIN:
